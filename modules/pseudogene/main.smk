...
ORFFINDER_CONFIG = MODULE_CONFIG.get("orffinder", {})
HMM_CONFIG = MODULE_CONFIG.get("hmm", {})
...
if PSEUDOGENE_FILTERING == 'yes':
    ...
    if GREP_CONFIG.get("type", config.get("grep_type", 1)) == 1:
        rule subset_taxonomy_by_taxon1:
            ...
            output:
                config["pipeline"]["output_dir"] + "/taxon.zotus"
            threads: 1
            resources:
                mem_mb = 1000,
                time_min = 5
    ...
    rule subset_ESV_sequences_by_taxon:
        ...
        output:
            config["pipeline"]["output_dir"] + "/chimera.denoised.nonchimeras.taxon"
        threads: 1
        resources:
            mem_mb = 2000,
            time_min = 10
    ...
    if config.get("removal_type", 1) == 1:
        ...
        rule get_orfs_nt_longest:
            ...
            input:
                config["pipeline"]["output_dir"] + "/chimera.denoised.nonchimeras.taxon"
            output:
                config["pipeline"]["output_dir"] + "/orfs.fasta.nt.longest"
            params:
        ...
        rule get_longest_orfs:
            ...
            input:
                config["pipeline"]["output_dir"] + "/orfs.fasta.nt.longest"
            output:
                config["pipeline"]["output_dir"] + "/longest.orfs.fasta"
            threads: 1
            resources:
                mem_mb = 1000,
                time_min = 10
        ...
        rule add_good_orfs_to_taxonomy_longest:
            ...
            input:
                ...
                rdp = config["pipeline"]["output_dir"] + "/rdp.out.tmp"
            output:
                temp(config["pipeline"]["output_dir"] + "/taxonomy_ORF.tsv")
            threads: 1
            resources:
                mem_mb = 1000,
                time_min = 5
        ...
        rule get_orfs_nt_hmm:
            ...
            input:
                config["pipeline"]["output_dir"] + "/chimera.denoised.nonchimeras.taxon"
            output:
                config["pipeline"]["output_dir"] + "/orfs.fasta.nt.hmm"
            params:
                g = ORFFINDER_CONFIG.get("g", 5),
                s = ORFFINDER_CONFIG.get("s", 2),
                ml = ORFFINDER_CONFIG.get("ml", 30),
        ...
        rule get_orfs_aa_hmm:
            ...
            input:
                config["pipeline"]["output_dir"] + "/chimera.denoised.nonchimeras.taxon"
            output:
                config["pipeline"]["output_dir"] + "/orfs.fasta.aa.hmm"
            params:
                g = ORFFINDER_CONFIG.get("g", 5),
                s = ORFFINDER_CONFIG.get("s", 2),
                ml = ORFFINDER_CONFIG.get("ml", 30),
        ...
        rule consolidate_orfs_hmm:
            ...
            input:
                nt = config["pipeline"]["output_dir"] + "/orfs.fasta.nt.hmm",
                aa = config["pipeline"]["output_dir"] + "/orfs.fasta.aa.hmm"
            output:
                nt2 = config["pipeline"]["output_dir"] + "/orfs.fasta.nt.filtered.hmm",
                aa2 = config["pipeline"]["output_dir"] + "/orfs.fasta.aa.filtered.hmm"
            threads: 1
            resources:
                mem_mb = 1000,
                time_min = 10
        ...
        rule hmmscan:
            ...
            input:
                ...
                hmm = HMM_CONFIG.get("path", config.get("hmm", "config/hmm/bold.hmm"))
            output:
                config["pipeline"]["output_dir"] + "/hmm.txt"
            threads: 1
            resources:
                mem_mb = 2000,
                time_min = 45
        ...
        rule add_good_orf_sequences_to_taxonomy_hmm:
            ...
            output:
                config["pipeline"]["output_dir"] + "/taxonomy_ORF.tsv"
            threads: 1
            resources:
                mem_mb = 1000,
                time_min = 10
...
checkpoint pseudogene_filtering_complete:
    ...
    input:
        ...
        config["pipeline"]["output_dir"] + "/ESV.table"
    output:
        touch(config["pipeline"]["output_dir"] + "/checkpoints/pseudogene_complete.done")
    message:
